...
func TestGormRefreshTokenRepository_RevokeAndReplacement(t *testing.T) {
	...
	t.Run("revoke by id not found", func(t *testing.T) {
		ctx, _ := newTestTx(t)
		repo := NewGormRefreshTokenRepository(testDB)
		require.ErrorIs(t, repo.RevokeByID(ctx, uuid.New()), repository.ErrNotFound)
	})
	...
	t.Run("revoke by user id not found", func(t *testing.T) {
		ctx, _ := newTestTx(t)
		repo := NewGormRefreshTokenRepository(testDB)
		require.ErrorIs(t, repo.RevokeByUserID(ctx, uuid.New()), repository.ErrNotFound)
	})
	...
	t.Run("revoke by session id not found", func(t *testing.T) {
		ctx, _ := newTestTx(t)
		repo := NewGormRefreshTokenRepository(testDB)
		require.ErrorIs(t, repo.RevokeBySessionID(ctx, uuid.New()), repository.ErrNotFound)
	})
	...
	t.Run("mark replacement not found", func(t *testing.T) {
		ctx, _ := newTestTx(t)
		repo := NewGormRefreshTokenRepository(testDB)
		require.ErrorIs(t, repo.MarkReplacement(ctx, uuid.New(), uuid.New()), repository.ErrNotFound)
	})
}

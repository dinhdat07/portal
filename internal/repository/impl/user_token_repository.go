...
func (r *GormUserTokenRepository) Create(ctx context.Context, token *model.UserToken) error {
	return r.getDB(ctx).Create(token).Error
}

func (r *GormUserTokenRepository) FindValidToken(ctx context.Context, tokenHash string, tokenType domain.TokenType) (*model.UserToken, error) {

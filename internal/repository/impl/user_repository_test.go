...
func TestGormUserRepository_FindByIDUnscoped(t *testing.T) {
	t.Run("finds soft-deleted user", func(t *testing.T) {
		...
		deletedBy := uuid.New()
		require.NoError(t, tx.Model(&model.User{}).Where("id = ?", user.ID).Updates(map[string]interface{}{
			"deleted_at": time.Now(),
...
func TestGormUserRepository_MarkEmailVerified(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		require.NoError(t, tx.Model(&model.User{}).Where("id = ?", user.ID).Update("status", domain.StatusPending).Error)
...
func TestGormUserRepository_ListUsers(t *testing.T) {
	t.Run("basic list with pagination", func(t *testing.T) {
		...
		users, total, err := repo.ListUsers(ctx, repository.UserListFilter{
			Page:     1,
			PageSize: 2,
		})
		require.NoError(t, err)
		...
		// second page
		users2, _, err := repo.ListUsers(ctx, repository.UserListFilter{
			Page:     2,
			PageSize: 2,
		})
		require.NoError(t, err)
	...
	t.Run("include deleted", func(t *testing.T) {
		...
		users, _, err := repo.ListUsers(ctx, repository.UserListFilter{
		...
		users2, total2, err := repo.ListUsers(ctx, repository.UserListFilter{
			Email:          "incldel@example.com",
			IncludeDeleted: true,
			Page:           1,
			PageSize:       10,
...
func mustCreateRole(t *testing.T, tx *gorm.DB, code domain.RoleCode) *model.Role {
	t.Helper()
...
func mustCreateUser(t *testing.T, tx *gorm.DB, roleID uuid.UUID, email, username string) *model.User {
	t.Helper()

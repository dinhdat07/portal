...
func TestUserService_UpdateProfile_Table(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			userRepo.EXPECT().FindByUsername(mock.Anything, newUsername).RunAndReturn(func(ctx context.Context, username string) (*model.User, error) {
...
func TestUserService_ChangePassword_Table(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			newPassword:     "   ",
			confirmPassword: "new-pass-123",
			user:            &model.User{ID: actorID, PasswordHash: ptrString(string(hashedCurrent))},
		...
		},
		{
			...
			newPassword:     "new-pass-123",
			confirmPassword: "new-pass-123",
			user:            &model.User{ID: actorID, PasswordHash: nil},
			expectedErr:     ErrUnauthorized,
		},
		{

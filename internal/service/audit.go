...
func (s *auditLogService) Log(ctx context.Context, meta *AuditMeta, action domain.ActionName, actor *AuditUser, target *AuditUser) error {
	log := &model.AuditLog{Action: action}
...
func (s *auditLogService) List(ctx context.Context, filter AuditLogFilter) ([]model.AuditLog, int64, error) {
	if filter.Page <= 0 {
		filter.Page = 1
	}
	...
	logs, total, err := s.repo.List(ctx, repository.AuditLogListFilter{

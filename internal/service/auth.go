...
func (s *authService) handleRefreshTokenReuse(ctx context.Context, meta *AuditMeta, reused *model.RefreshToken) error {
	...
	err = s.txManager.WithTx(ctx, func(txCtx context.Context) error {
		if err := s.sessionRepo.RevokeAllByUserID(txCtx, reused.UserID); err != nil && !errors.Is(err, repository.ErrNotFound) {
			return err
		}

		if err := s.refreshRepo.RevokeByUserID(txCtx, reused.UserID); err != nil && !errors.Is(err, repository.ErrNotFound) {
			return err
		}
...
func (s *authService) Logout(ctx context.Context, meta *AuditMeta, actor *AuditUser, sessionID uuid.UUID) error {
	...
	err = s.txManager.WithTx(ctx, func(txCtx context.Context) error {
		if err := s.sessionRepo.RevokeByID(txCtx, sessionID); err != nil && !errors.Is(err, repository.ErrNotFound) {
			return err
		}
...
func (s *authService) LogoutAll(ctx context.Context, meta *AuditMeta, actor *AuditUser) error {
	...
	err = s.txManager.WithTx(ctx, func(txCtx context.Context) error {
		if err := s.sessionRepo.RevokeAllByUserID(txCtx, actor.ID); err != nil && !errors.Is(err, repository.ErrNotFound) {
			return err
		}
